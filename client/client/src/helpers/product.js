  ...
  if (products && sortType && sortValue) {
    if (sortType === "category") {
      ...
      const filteredProducts = products.filter(product => {
        ...
        const productCategory = getProductCategoryName(product?.productCategories);
...
const fetchCategoryById = async (categoryId) => {
  try {
    const response = await axios.get(`http://localhost:5000/api/v1/product/cat/${categoryId}`);
